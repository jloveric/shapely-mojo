...
from shapely.geometry import LinearRing, Polygon, Point, LineString, MultiPolygon, MultiLineString, GeometryCollection
from shapely.measurement import distance as _distance
from shapely.predicates import intersects as _intersects, overlaps as _overlaps, touches as _touches, contains as _contains, covers as _covers, contains_properly as _contains_properly
...
struct STRtree:
    ...
    fn _qsort_geom_slice_by_miny(self, idx: List[Int32], start: Int, end_excl: Int):
    ...
    fn query(self, _target: Geometry, predicate: String) -> List[Geometry]:
        ...
        if predicate == "intersects":
            var out = List[Geometry]()
            for g in cands:
                if _intersects(g, _target): out.append(g.copy())
            return ^out
        elif predicate == "touches":
            var out2 = List[Geometry]()
            for g in cands:
                if _touches(g, _target): out2.append(g.copy())
            return ^out2
        elif predicate == "overlaps":
            var out3 = List[Geometry]()
            for g in cands:
                if _overlaps(g, _target): out3.append(g.copy())
            return ^out3
        elif predicate == "contains":
            var out4 = List[Geometry]()
            for g in cands:
                if _contains(g, _target): out4.append(g.copy())
            return ^out4
        elif predicate == "within":
            var out5 = List[Geometry]()
            for g in cands:
                if _contains(_target, g): out5.append(g.copy())
            return ^out5
        elif predicate == "covers":
            var out6 = List[Geometry]()
            for g in cands:
                if _covers(g, _target): out6.append(g.copy())
            return ^out6
        elif predicate == "covered_by":
            var out7 = List[Geometry]()
            for g in cands:
                if _covers(_target, g): out7.append(g.copy())
            return ^out7
        elif predicate == "contains_properly":
            var out10 = List[Geometry]()
            for g in cands:
                if _contains_properly(g, _target): out10.append(g.copy())
            return ^out10
        return ^cands
    ...
    fn query_bulk(self, _targets: List[Geometry]) -> List[Tuple[Int32, Int32]]:
        ...
        while ti < _targets.size():
            ...
            var gi = 0
            while gi < idxs.size():
                pairs.append((ti as Int32, idxs[gi]))
                gi += 1
            ti += 1
        return ^pairs
